...
;	SetBatchLines, 1
	...
	Gui, Show, hide x110 y151 h450,% hthisTEXTid a_space hthisTEXTid+0  
	return
...
start:
 ...
 Sonnenstunden 		:= ConvertToMilliseconds(x.2 ":" A_Sec "`," A_MSec) - ConvertToMilliseconds(x.1 ":" A_Sec "`," A_MSec)
	...
	thisDebug := 0
	if thisDebug {
 Sonnenaufgang 		:= ConvertToMilliseconds(A_Hour ":" A_Min ":" A_Sec "`," A_MSec) - ConvertToMilliseconds(A_Hour ":" A_Min-2 ":" A_Sec "`," A_MSec)
 Sonnenuntergang 	:= ConvertToMilliseconds(A_Hour ":" A_Min-5 ":" A_Sec "`," A_MSec)
	SAstd 	 := SAstdNoLTRIM := subStr(ConvertToHHMMSSMS(Sonnenaufgang),1,2)
	SAstd 	 := LTRIM(SAstd,0)
	SAmin	 := SAminNoLTRIM := subStr(ConvertToHHMMSSMS(Sonnenaufgang),4,2)
	SAmin	 := LTRIM(SAmin,0)
 ...
 thisGUIUpdate := "Aktuelle Zeit: " A_DD "." Month(A_MM) "." a_YYYY "  " A_Hour ":" A_Min ":" A_Sec " Uhr"
		...
		. "`n" AThisHotkey() "GUI Timeout: " thisGUITimeout(thisSleep)
	...
	. "`nZeit bis zum nächsten Sonnenaufgang:`t`t" 
		...
		? SAmin " Minuten"
		: (SAstd="") && (SAmin="")
		? thisRISE(RISEfile_) 			; Bei Sonnenaufgang wird RISEfile gestartet
		: "")
	. "`nZeit bis zum nächsten Sonnenuntergang:`t" 
		...
		? SUmin " Minuten"
		: (SUstd="") && (SUmin="")
		? thisSET(SETfile_) 			; Bei Sonnenuntergang wird SETfile gestartet
		: "")
		. wetterDaten()
	else {
		DayCounter++
...
atickcount := a_tickcount
;ToggleGUI := (ToggleGUI:=!ToggleGUI)
ToggleGUI := 0
		Gui, Show, x110 y151,% hthisTEXTid a_space hthisTEXTid+0  
		if !ToggleGUI
GUItimeout(122.25) ; Zwei Stunden, zwei Minuten und fünfzehn Sekunden entsprechen wieviel minuten
return

F4::  ; test
ToolTip % (timeStampRISEall ? timeStampRISEall : "empty") "`n"
	. (timeStampSETall  ? timeStampSETall  : "empty")
sleep,% (.08*60*1000)
ToolTip
return
...
update:
if ToggleGUI 
GuiControl,,% hthisTEXTid, %thisGUIUpdate%
return
...
thisRISE(RISEfile) {  
	...
	return " `tSUNRISE"
		...
		thisRISECounter++
		if !ToggleGUI
		GUItimeout(1.1) ; .1 ist 6 sekunden
		return " `tSUNRISE"
		} else {
			Gui, Show, x110 y151,% hthisTEXTid a_space hthisTEXTid+0  
			try run,% RISEfile,, hide, AusgabeVarPID
