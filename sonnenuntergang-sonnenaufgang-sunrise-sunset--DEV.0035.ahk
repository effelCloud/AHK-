...
; zoomfactor:=((A_ScreenWidth/A_ScreenWidth)*(96/A_ScreenDPI))
;zoomfactor:=((A_ScreenWidth/1360)*(96/A_ScreenDPI)) ; doros rechner
...
Gui, Font, s%fs11% normal,
	...
	Gui, Add, DropDownList, x%x% y%y% w%w% r15 section vddl1 choose1, %a_space%Trigger auswählen...
								| vor Sonnenaufgang
								| nach Sonnenaufgang
								| vor Sonnenuntergang
	...
	Gui, Add, DropDownList,x%x% y%y% w%w% vddl3 choose1,  %a_space%Trigger Datei auswählen...
								| Öffnen...
...
Gui, Font, s%fs11% normal,							; Nächste geplante Aktion
	x:=(wa*1)/xx,	y:=(ha*41)/xx,	w:=(wa*28)/xx,	h:=(ha*4.2)/xx
	Gui, Add, GroupBox, x%x% y%y% w%w% h%h%, %a_Space%Nächste System Aktion:%a_Space%
	x:=(wa*11)/xx,	y:=(ha*42.7)/xx,	w:=(wa*15)/xx,	h:=(ha*2.2)/xx
	Gui, Add, text, x%x% y%y% w%w% h%h% vNächsteGeplanteAction, % MainArray["nextScheduledAction"]
;	Gui, Add, GroupBox, x%x% y%y% w%w% h%h% vNächsteGeplanteAction
;-	GuiControl,, NächsteGeplanteAction,% MainArray["nextScheduledAction"]

Gui, Font, s%fs11% normal,							; 
	x:=(wa*1)/xx,	y:=(ha*46.5)/xx,	w:=(wa*28)/xx,	h:=(ha*2.11)/xx
	Gui, Add, GroupBox, x%x% y%y% w%w% h%h% , %a_Space%Log:%a_Space%

Gui, Font, s%fs11% normal,							; 
	x:=(wa*1)/xx,	y:=(ha*48.5)/xx,	w:=(wa*28)/xx,	h:=(ha*20)/xx
	Gui, Add, listview, x%x% y%y% w%w% h%h%  vLV1 hwndhLV1 grid background%color% cBlack , Timestamp|ThisLabel|Name|ThisFunc|ThisHotkey|PriorHotkey|PriorKey|TimeSinceThisHotkey|TimeSincePriorHotkey|GuiControl|GuiEvent|EventInfo
...
;	x:=(wa*0)/xx,	y:=(ha*1)/xx,	w:=(wa*30)/xx  ;- w ist gui width  
	x:=(wa*1)/xx,	y:=(ha*8.5)/xx,	w:=(wa*28)/xx
	Gui, Add, DropDownList,x%x% y%y% w%w% vddl3 +center choose1,  Trigger Datei auswählen...| Öffnen...
	;-
	color=D4D0C8 
...
;	Gui, Add, listview, x10 ym+390 	w545 r7  vLV1 hwndhLV1 grid, Timestamp|ThisLabel|Name|ThisFunc|ThisHotkey|PriorHotkey|PriorKey|TimeSinceThisHotkey|TimeSincePriorHotkey|GuiControl|GuiEvent|EventInfo

	x:=(wa*1)/xx,y:=(ha*43)/xx,w:=(wa*28)/xx,h:=(ha*20)/xx
	Gui, Add, listview, x%x% y%y% w%w% h%h%  vLV1 hwndhLV1 grid, Timestamp|ThisLabel|Name|ThisFunc|ThisHotkey|PriorHotkey|PriorKey|TimeSinceThisHotkey|TimeSincePriorHotkey|GuiControl|GuiEvent|EventInfo
	T1:=(wa*1.9)/xx,T2:=(wa*1.9)/xx,T3:=(wa*6)/xx,T4:=(wa*17)/xx
;    LV_ModifyCol(1,T1),LV_ModifyCol(2,T2),LV_ModifyCol(3,T3),LV_ModifyCol(4,T4)
...
;msgbox,,% a_lineNumber, % "showMsgBoxTest#854a = " showMsgBoxTest#854a "`n" test#854a
	...
	if Trigger#2
	{
		...
		MainArray["sollZeit"] 	:= 00 ":" 10 ":" 01
			...
			if (MainArray["istZeit"] = MainArray["sollZeit"])
			{
