...
start:
	...
	if getBevor
	{
 	Sonnenaufgang 	:= ConvertToMilliseconds(x.1  ":" 00 "`," 000)
			 - ConvertToMilliseconds(A_Hour ":" A_Min ":" 00 "`," 000)
 	Sonnenuntergang := ConvertToMilliseconds(x.2   ":" 00 "`," 000)
			 - ConvertToMilliseconds(A_Hour ":" A_Min ":" 00 "`," 000)
;	Sonnenstunden 	:= ConvertToMilliseconds((MainArray["thisCalendarDaySunSet",A_DD,A_MM])  ":" 00 "`," 000)
...
/*
	sauf 	:= ConvertToMilliseconds((MainArray["thisCalendarDaySunRise",A_DD,A_MM]) ":" 00 "`," 000)
	sunter 	:= ConvertToMilliseconds((MainArray["thisCalendarDaySunSet",A_DD,A_MM]) ":" 00 "`," 000)
	saufA 	:= ConvertToMilliseconds(x.1  ":" 00 "`," 000)
	sunterA	:= ConvertToMilliseconds(x.2  ":" 00 "`," 000)
	MainArray["nightOrDay"] := (sunter<sauf) ? "Night_x" : "Day_x"
	MainArray["nightOrDayA"] := (sunter>sauf) ? "Day_A" : "Night_A"
	MainArray["nightOrDayB"] := (sunterA<saufA) ? "Day_B" : "night_B"
	MainArray["nightOrDayC"] := (sunterA>saufA) ? "Night_C" : "day_C"
*/
	...
	HrDelaySAstd 	 := SAstdNoLTRIM := subStr(ConvertToHHMMSSMS(Sonnenaufgang),1,2)
	HrDelaySAstd 	 := LTRIM(SAstd,0)
	HrDelaySAmin	 := SAminNoLTRIM := subStr(ConvertToHHMMSSMS(Sonnenaufgang),4,2)
	HrDelaySAmin	 := LTRIM(SAmin,0)
	HrDelaySUstd	 := SUstdNoLTRIM := subStr(ConvertToHHMMSSMS(Sonnenuntergang),1,2)
	HrDelaySUstd 	 := LTRIM(SUstd,0)
	HrDelaySUmin	 := SUminNoLTRIM := subStr(ConvertToHHMMSSMS(Sonnenuntergang),4,2)
	HrDelaySUmin	 := LTRIM(SUmin,0)
	...
	MainArray["Sonnenminuten"] 	:= ((StimeMIN=1)
					...
					? StimeMIN " Minuten"
					: "") 
	MainArray["Stunden bis zum naechsten Sonnenaufgang"]	:= ((SAstd=1)
								? "  " SAstd " Stunde    "
								: (SAstd>1) && (SAstd<10)
								? "  " SAstd " Stunden "
								: (SAstd>=10)
								? SAstd " Stunden "
								: "") 
	MainArray["Minuten bis zum naechsten Sonnenaufgang"]	:= ((SAmin=1)
								? "  " SAmin " Minute"
								: (SAmin>1) && (SAmin<10)
								? "  " SAmin " Minuten"
								: (SAmin>=10)
								...
								? thisRISE(RISEfile)
								: "")
	MainArray["Stunden bis zum naechsten Sonnenuntergang"]	:= ((SUstd=1)
								? "  " SUstd " Stunde    "
								: (SUstd>1) && (SUstd<10)
								? "  " SUstd " Stunden "
								: (SUstd>=10)
								? SUstd " Stunden "
								: "") 
	MainArray["Minuten bis zum naechsten Sonnenuntergang"]	:= ((SUmin=1)
								? "  " SUmin " Minute"
								: (SUmin>1) && (SUmin<10)
								? "  " SUmin " Minuten"
								: (SUmin>=10)
...
/*
	MainArray["Stunden seit letztem Sonnenaufgang"]		:= ((HrDelaySAstd=1)
								? "  " HrDelaySAstd " Stunde    "
								: (HrDelaySAstd>1) && (HrDelaySAstd<10)
								? "  " HrDelaySAstd " Stunden "
								: (HrDelaySAstd>=10)
								? HrDelaySAstd " Stunden "
								: "") 
	MainArray["Minuten seit letztem Sonnenaufgang"]		:= ((HrDelaySAmin=1)
								? "  " HrDelaySAmin " Minute"
								: (HrDelaySAmin>1) && (HrDelaySAmin<10)
								? "  " HrDelaySAmin " Minuten"
								: (HrDelaySAmin>=10)
								? HrDelaySAmin " Minuten"
								: (HrDelaySAstd="") && (HrDelaySAmin="")
								? thisRISE(RISEfile)
								: "")
	MainArray["Stunden seit letztem Sonnenuntergang"]	:= ((HrDelaySUstd=1)
								? "  " HrDelaySUstd " Stunde    "
								: (HrDelaySUstd>1) && (HrDelaySUstd<10)
								? "  " HrDelaySUstd " Stunden "
								: (HrDelaySUstd>=10)
								? HrDelaySUstd " Stunden "
								: "") 
	MainArray["Minuten seit letztem Sonnenuntergang"]	:= ((HrDelaySUmin=1)
								? "  " HrDelaySUmin " Minute"
								: (HrDelaySUmin>1) && (HrDelaySUmin<10)
								? "  " HrDelaySUmin " Minuten"
								: (HrDelaySUmin>=10)
								? HrDelaySUmin " Minuten"
								: (HrDelaySUstd="") && (HrDelaySUmin="")
								? thisSET(SETfile)
								: "")
*/
	...
	MainArray["Zeit seit letztem Sonnenuntergang_"]	 := 	  ConvertToMilliseconds(MainArray["thisCalendarDaySunSet",A_DD,A_MM] ":" 00 "`," 000)
...
*/
 if !MakeList
 {
 thisGuiUpdate := "Aktuelle Zeit: " MainArray["Aktuelle Zeit"] " - NightOrDay: " MainArray["nightOrDay"] "   [ " MainArray["tagOderNacht"] " ]" 
;	. "`nRISE#: " MainArray["thisRISECounter"] " - SET#: " MainArray["thisSETCounter"] " - " "MOD: " thisToggleFunc " - " thisMOD " - " MainArray["nightOrDay"] 
;	. "`nZeit seit letztem Sonnenuntergang:`t" MainArray["Zeit seit letztem SonnenuntergangHHMMSSMS"] ; debug
;	. "`nZeit seit letztem Sonnenaufgang:`t" MainArray["Zeit seit letztem SonnenaufgangHHMMSSMS"] ; debug
	...
	. "`nZeit seit letztem Sonnenaufgang:`t`t" MainArray["Zeit seit letztem SonnenaufgangHHMM"]
	...
	. "`nZeit seit letztem Sonnenuntergang:`t`t" MainArray["Zeit seit letztem SonnenuntergangHHMM"]
	} else {
		DayCounter++
...
atickcount := a_tickcount
GuiLiveTime:=(1439.99*60*1000)
; return "`nGUI Timeout:`t`tdisabled - press [ F3 ] to activate - switch [ F5 ]"
