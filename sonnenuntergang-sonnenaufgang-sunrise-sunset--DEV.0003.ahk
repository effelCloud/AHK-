...
; https://api.openweathermap.org/data/2.5/onecall?lat=54.328074446743706&lon=10.116245523261876&appid=5cc7ada2fb0ee27627418a8e5e93193a&units=metric&lang=de
; http://api.openweathermap.org/data/2.5/air_pollution?appid=5cc7ada2fb0ee27627418a8e5e93193a&units=metric&lang=de&lat=54.328074446743706&lon=10.116245523261876
...
*/
	...
	GuiLiveTime := (1439.99*60*1000) ; GuiLiveTime bei Guistart
	ShowGuiLiveTime := 1
	...
	Gui, Font, 	s12
	Gui, Add, text, xs y+13 w550 h240 vEDIT1 gEDIT1
	Gui, Show, x110 y11 w555 h245,% hGUIA  
	return
...
EDIT1:
	...
	MainArray["GUI LiveTimes"] 	:= "`nLiveTimes:`t`t{"
 					. subStr(ConvertToHHMMSSMS(GuiLiveTime),1,8) " / "		; TimeStamp
 					. subStr(ConvertToHHMMSSMS(GuiLiveTime-atickcountB),1,8) " / " 	; downCount
...
{
Gui, Show, ,% hthisTEXTid a_space hthisTEXTid+0  
atickCount := a_tickCount
GuiLiveTime:=(.11*60*1000)
thisGUITimeout()
thisRISE(RISEfile)
;ToggleGUI := 1
} else {
atickCount := a_tickCount
ToolTip
GuiLiveTime:=(.001*60*1000)
thisGUITimeout()
Gui, Show, hide ,% hthisTEXTid a_space hthisTEXTid+0  
;ToggleGUI := 0
}
return
...
thistoolTipx := strReplace(thistoolTip,"nächsten Sonnenaufgang:`t`t","nächsten Sonnenaufgang:`t")
;tooltip,% thistoolTipx
} else {
ToolTip
Gui, Show, hide ,% hthisTEXTid a_space hthisTEXTid+0  
}
...
global
if ShowGuiLiveTime
return    (thisSTARTCountTimeout() 
	? "`nGUI DefaultTime:`t" thisSTARTCountTimeout()
...
Month(thisMonth) {
	...
	return " `tSUNRISE"
 	;	atickCount := a_tickCount
		...
		thisRISECounter++
	;	if !ToggleGUI
	;	ToolTipTimeout(1.1) ; .1 ist 6 sekunden
	;	thisGuiTimeOutDownCounter(360000)
		return " `tSUNRISE"
}
