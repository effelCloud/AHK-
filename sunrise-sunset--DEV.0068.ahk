...
;////////////////////////////////////////////////////////////////////////////////////////////////////////
;22 Wie entferne ich den gepunkteten Rand um das Steuerelement 
...
return
;////////////////////////////////////////////////////////////////////////////////////////////////////////
GuiContextMenu() { ; https://ahkde.github.io/docs/v1/lib/Gui.htm#GuiContextMenu ;zurück
global
gui, submit, nohide
;msgbox,4096,, % thisClipboard := ":Alternativ > Menü oder Umschalt+F10 <`n" 
thisClipboard := "Alternativ: > Menü oder Umschalt+F10 <`n" 
	. "a_lineNumber`t`t:`t" a_lineNumber "`n"
	. "A_Gui:`t`t`t`t" A_Gui "`n"
	. "A_DefaultListView:`t`t" A_DefaultListView "`n"
	. "A_DefaultGui:`t`t`t" A_DefaultGui "`n"
	. "A_GuiControl:`t`t`t" A_GuiControl "`n"
	. "A_GuiWidth:`t`t`t" A_GuiWidth "`n"
	. "A_GuiHeight:`t`t`t" A_GuiHeight "`n"
	. "A_GuiX:`t`t`t`t" A_GuiX "`n"
	. "A_GuiY:`t`t`t`t" A_GuiY "`n"
	. "A_GuiEvent:`t`t`t" A_GuiEvent "`n"
	...
	. thisTT
;	. ":`t`t"  "`n"
	setTimer, thisTMPListView, -1
}

thisTMPListView: ;zurück
	atickCount := a_tickCount
	Gui -DPIScale hwndh%atickCount% 
	gui, h%atickCount%: Add, Listview, r35 w1100 grid vTMPListView hwndHTMPListView gTMPListView, #|ControlID|Value
	; Erstellen Sie eine neue Instanz von LV_InCellEdit für HTMPListView
	ICELV1 := New LV_InCellEdit(HTMPListView, True, True)
	; Bearbeitbare Spalten einschränken
	;ICELV1.SetColumns(1, 2, 3, 4, 5)

	Gui, h%atickCount%: Font,							% thisZoomFactorNormal(12.01)
	GuiControl, h%atickCount%:    MoveDraw, 		TMPListView, 		% thisX(1.0) thisY(1.7) thisW(85.0) thisH(80.0) ;  default
	GuiControl, h%atickCount%:    Font, 			TMPListView
;	GuiControl, h%atickCount%: ,  TMPListView, 					; empty

	gui, h%atickCount%: default
	gui, h%atickCount%: listview, TMPListView
	LV_ADD("", 0, "GUI hwnd", "h" atickCount)
	Loop, parse,% thisClipboard, `n
	 {
	 if a_loopField {
	 if (a_index=31)
	 LV_ADD("", (a_index), "_TT",a_loopField)
	 else
	 LV_ADD("", (a_index), (strSplit(a_loopField,":",a_tab))*)
	 }}
	Loop, % LV_GetCount("Columns")
	LV_ModifyCol(A_Index, "AutoHdr")


	x:=(wa*2)/xx,	y:=(ha*10)/xx,	 w:=(wa*85)/xx,	h:=(ha*80)/xx
	gui, h%atickCount%: show, AutoSize Center,% strReplace((substr(a_scriptname,-11)),".ahk") "   hwnd H" atickCount
	ICELV1.OnMessage()
	gui, 1: listview, listview2
	gui, 1: default
return
...
;SubLV1:
_TMPListView:
; Verwenden Sie die Taste „e“, um ggf. die erste bearbeitbare Zelle der fokussierten Zeile zu bearbeiten
	gui, h%atickCount%: default
	gui, h%atickCount%: listview, TMPListView
If (A_GuiEvent == "K") && (Chr(A_EventInfo) = "e") {
   Gui, h%atickCount%: ListView, %A_GuiControl%
...
global thisTT ;zurück
	...
	if AktStrlmnt
;	ToolTip % thisTT := %AktStrlmnt%_TT "`n" SteuerelementIDData(AktStrlmnt)
	thisTT := %AktStrlmnt%_TT "`n" SteuerelementIDData(AktStrlmnt)
	SetTimer, EntferneToolTip, -1 ;  % (.0001*60*1000)
	return
	EntferneToolTip:
...
; ///////////////////////////////////////////////////////////////////////////////////////////
; /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
; KLASSE LV_InCellEdit
;
; Im Gegensatz zu anderen In-Cell-Bearbeitungsskripts verwendet diese Klasse das integrierte Bearbeitungssteuerelement von ListViews.
; Vorteil:
; Sie müssen sich nicht um die Schriftart und die grafische Benutzeroberfläche kümmern, und der größte Teil der Arbeit kann durch die Handhabung von Common ListView erledigt werden
; Benachrichtigungen.
; Nachteil:
; Ich habe immer noch keine Möglichkeit gefunden, zu verhindern, dass ListView beim Bearbeiten das erste Unterelement der Zeile ausblendet
; ein weiterer Unterpunkt. Die einzige bekannte Problemumgehung besteht darin, eine ausgeblendete erste Spalte hinzuzufügen.
;
; Die Klasse bietet Methoden, um die Bearbeitung auf bestimmte Spalten zu beschränken, um direkt mit der Bearbeitung einer bestimmten Zelle zu beginnen,
; und um den integrierten Nachrichtenhandler für WM_NOTIFY-Nachrichten zu deaktivieren/aktivieren (siehe unten).
;
; Der Nachrichtenhandler für WM_NOTIFY-Nachrichten wird für die angegebene ListView aktiviert, wenn eine neue Instanz vorhanden ist
; erstellt. Solange der Nachrichtenhandler aktiviert ist, wird durch einen Doppelklick auf eine beliebige Zelle ein Bearbeitungssteuerelement darin angezeigt
; Zelle, die es ermöglicht, den aktuellen Inhalt zu bearbeiten. Das Standardverhalten zum Bearbeiten der ersten Spalte durch zwei aufeinanderfolgende Einzelspalten
; Klicks ist deaktiviert. Sie müssen „Esc“ drücken, um die Bearbeitung abzubrechen, andernfalls wird der Inhalt der Bearbeitung gespeichert
; die aktuelle Zelle. ListViews müssen über die Option -ReadOnly verfügen, um bearbeitet werden zu können.
;
; Während der Bearbeitung werden die Tasten „Esc“, „Tab“, „Umschalt+Tab“, „Ab“ und „Auf“ als Hotkeys registriert. Mit „Esc“ wird die Bearbeitung abgebrochen
; ohne den Wert der aktuellen Zelle zu ändern. Alle anderen Hotkeys speichern den Inhalt der Bearbeitung im aktuellen
; Zelle und setzen Sie die Bearbeitung für die nächste (Tab), vorherige (Umschalt+Tab), obere (oben) oder untere (unten) Zelle fort. Sie können es nicht verwenden
; Sie können die Tasten während der Bearbeitung auch für andere Zwecke verwenden.
;
; Alle Änderungen werden in MyInstance.Changed gespeichert. Sie können die Änderungen verfolgen, indem Sie (A_GuiEvent == "F") im auslösen
; ListViews gLabel und Überprüfung von MyInstance["Changed"], wie im Beispielskript gezeigt. Wenn „True“, MyInstance.Changed
; enthält ein Array von Objekten mit den Schlüsseln „Row“ (Zeilennummer), „Col“ (Spaltennummer) und „Txt“ (neuer Inhalt).
; Changed ist einer der beiden Schlüssel, auf die direkt von außerhalb der Klasse zugegriffen werden soll.
;
; Wenn Sie die Bearbeitung in der Zelle vorübergehend deaktivieren möchten, rufen Sie MyInstance.OnMessage(False) auf. Dies muss auch vorher erfolgen
; Sie versuchen, die Instanz zu zerstören. Um es wieder zu aktivieren, rufen Sie MyInstance.OnMessage() auf.
;
; Um den Verlust von GUI-Ereignissen und -Nachrichten zu vermeiden, muss der Nachrichtenhandler möglicherweise „kritisch“ sein. Das kann sein
; Dies wird erreicht, indem die Instanzeigenschaft „Critical“ auf den erforderlichen Wert gesetzt wird (z. B. MyInstance.Critical := 100).
; Neue Instanzen sind standardmäßig auf „Kritisch, Aus“ eingestellt. Obwohl dies manchmal erforderlich ist, können ListViews oder die gesamte GUI erforderlich sein
; reagiert unter bestimmten Umständen nicht, wenn Critical festgelegt ist und die ListView über ein G-Label verfügt.
; =============================================== =============================================== ==================
    ...
    ; ++++++++++++++++++++++++++++++++++++++++++++++++++ ++++++++++++++++++++++++++++++++++++++++++++++++++ +++++++++++++++
    ; =============================================== =============================================== ===============
    ; __New() Erstellt eine neue LV_InCellEdit-Instanz für die angegebene ListView.
    ; Parameter: HWND – HWND von ListView
    ; Optional ------------------------------------------------- -----------------------------------------
    ; HiddenCol1 – ListView mit ausgeblendeter erster Spalte
    ; Werte: Wahr/Falsch
    ; Standard: Falsch
    ; BlankSubItem – Text des Unterelements beim Bearbeiten ausblenden
    ; Werte: Wahr/Falsch
    ; Standard: Falsch
    ; EditUserFunc – Der Name einer benutzerdefinierten Funktion, von der aus aufgerufen werden soll
    ; LVN_BEGINEDITLABEL und LVN_ENDEDITLABEL.
    ; Die Funktion muss mindestens 6 Parameter akzeptieren:
    ; Status – Der Status des Bearbeitungsvorgangs: BEGIN/END
    ; HLV – Das Handle für die ListView.
    ; HED – Das Handle für das Bearbeitungssteuerelement.
    ; Zeile – Die Zeilennummer des bearbeiteten Elements.
    ; Col - Der
    ; ​Spaltennummer des bearbeiteten Elements.
    ; Text – Der Text des bearbeiteten Elements vor/nach der Bearbeitung.
    ; Um den Verlust von Nachrichten zu vermeiden, sollte die Funktion so schnell wie möglich zurückkehren.
    ; =============================================== =============================================== ===============
Class LV_InCellEdit {
   ; Instance properties -----------------------------------------------------------------------------------------------
   ; +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
