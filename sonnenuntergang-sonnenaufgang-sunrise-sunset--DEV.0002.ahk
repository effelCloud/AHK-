...
/*
https://www.google.com/search?q=site%3Aautohotkey.com+alexa+Triggercmd&rlz=1C1CHBD_deDE898DE898&oq=site%3Aautohotkey.com+alexa+Triggercmd&aqs=chrome..69i57j69i58.10224j0j4&sourceid=chrome&ie=UTF-8
https://www.amazon.de/VanderMey-Consulting-LLC-TRIGGERcmd/dp/B06XFN2TZN/ref=cm_cr_arp_d_product_top?ie=UTF8
...
*/
	...
	; Skript das jeden Tag bei Sonnenuntergang ausgeführt wird
	; https://sonnenuntergang-sonnenaufgang.info/koeln ; hier findest du die Daten für deinen Standort
	; https://www.autohotkey.com/boards/viewtopic.php?p=439038#p439038 , by flyingDman
...
;	SetBatchLines, 1
	...
	settimer, start, -1
	settimer, update, 300
	settimer, gefahrenmeldung_flash, 500
	toggleF3 := 0
	Gui, Font, s14
	Gui, Add, text, xs y+10 w550 h30 vEDIT2 hwndhthisTEXTtest cblue greload center,% "ACHTUNG Gefahrenmeldung Quelle: < Deutscher Wetterdienst >`n" 
	Gui, Font, s12
	Gui, Add, text, xs y+10 w550 h350 vEDIT1 hwndhthisTEXTid cred greload
	Gui, Show, x110 y151 h450,% hthisTEXTid a_space hthisTEXTid+0  
...
;	atickcount := a_tickcount
	thisDebugA_NOWminus2Minuten := ConvertToMilliseconds(A_Hour ":" A_Min ":" A_Sec "`," A_MSec)
	thisDebugA_NOWminus2Minuten -= (2*60*1000)
	return
...
start:
 ...
 Sonnenaufgang 		:= ConvertToMilliseconds(x.1 ":" A_Sec "`," A_MSec) - ConvertToMilliseconds(A_Hour ":" A_Min ":" A_Sec "`," A_MSec)
 Sonnenuntergang 	:= ConvertToMilliseconds(x.2 ":" A_Sec "`," A_MSec) - ConvertToMilliseconds(A_Hour ":" A_Min ":" A_Sec "`," A_MSec)
 Sonnenstunden 		:= ConvertToMilliseconds(x.2 ":" A_Sec "`," A_MSec) - ConvertToMilliseconds(x.1 ":" A_Sec "`," A_MSec)
	...
	thisDebug := 0
	if thisDebug {
	settimer, RUNtooltip, 100
 Sonnenaufgang 		:= ConvertToMilliseconds(A_Hour ":" A_Min ":" A_Sec "`," A_MSec)  - thisDebugA_NOWminus2Minuten 
 Sonnenuntergang 	:= ConvertToMilliseconds(A_Hour ":" A_Min ":" A_Sec "`," A_MSec) ; - thisDebugA_NOWminus2Minuten 
	SAstd 	 := SAstdNoLTRIM := subStr(ConvertToHHMMSSMS(Sonnenaufgang),1,2)
	SAstd 	 := LTRIM(SAstd,0)
	SAmin	 := SAminNoLTRIM := subStr(ConvertToHHMMSSMS(Sonnenaufgang),4,2)
	SAmin	 := LTRIM(SAmin,0)
 ...
 thisGUIUpdate := "Aktuelle Zeit: " A_DD "." Month(A_MM) "." a_YYYY "  " A_Hour ":" A_Min ":" A_Sec " Uhr"
		. "    RISE#: " thisRISECounter "  SET#: " thisSETCounter
		. thisGuiTimeOutDownCounter()
	. "`nSonnenaufgang:`t" x.1 "`tUhr"
	. "`nSonnenuntergang:`t" x.2 "`tUhr"
	. "`nSonnenstunden:`t`t`t`t" 
	. StimeStd " Stunden " 
		. ((StimeMIN=1)
		? "  " StimeMIN " Minute"
	...
	. "`nZeit bis zum nächsten Sonnenaufgang:`t`t" 
		...
		? SAmin " Minuten"
		: (SAstd="") && (SAmin="")
		? thisRISE(RISEfile) 			; Bei Sonnenaufgang wird RISEfile gestartet
		: "")
	. "`nZeit bis zum nächsten Sonnenuntergang:`t" 
		...
		? SUmin " Minuten"
		: (SUstd="") && (SUmin="")
		? thisSET(SETfile) 			; Bei Sonnenuntergang wird SETfile gestartet
		: "")
		. wetterDaten()
	else {
		DayCounter++
...
wetterDaten() {  ; test - original API daten vom Freitag.13.10.2023
	return  "`n`nend: 1697184000 <> 1697256"
		. "`nevent: Windböen"
		. "`nsender_name: Deutscher Wetterdienst"
		. "`nstart: 1697184000 <> 1697256"
		. "`ntags: Wind"
		. "`n`ndescription: Es besteht die Gefahr von Windböen (Stufe 1 von 4).Max. Böen: 50-60 km/h; Windrichtung: Südwest; Erhöhte Böen: an exponierten Standorten ~ 70 km/h"
		}

thisGuiTimeOutDownCounter(thisSleep:=1) {
global
if thisSleep
return  "`nGUI Timeout: " thisGUITimeout(thisSleep)
else
return ""
}
...
return AThisHotkey " gedrückt - "
}

thisGUITimeout(thisSleep) {
 global 
 atickcountB := a_tickcount-atickcount
 thisX := subStr(ConvertToHHMMSSMS(thisSleep-atickcountB),1,8) 	; downCount
	thisSleepStd := subStr(thisX,1,2)
	thisSleepStd := LTRIM(thisSleepStd,0)
	thisSleepMIN := subStr(thisX,4,2)
	thisSleepMIN := LTRIM(thisSleepMIN,0)
	thisSleepSEC := subStr(thisX,7,2)
	thisSleepSEC := LTRIM(thisSleepSEC,0)
 return   	((thisSleepSTD=1)
		? "  " thisSleepSTD " Stunde "
		: (thisSleepSTD>1) && (thisSleepSTD<10)
		? "  " thisSleepSTD " Stunden "
		: (thisSleepSTD>=10)
		? thisSleepSTD " Stunden "
		: "") 
		. ((thisSleepMIN=1)
		? thisSleepMIN " Minute "
		: (thisSleepMIN>1) && (thisSleepMIN<10)
		? "  " thisSleepMIN " Minuten "
		: (thisSleepMIN>=10)
		? thisSleepMIN " Minuten "
		: "") 
		. ((thisSleepSEC=1)
		? thisSleepSEC " Sekunde"
		: (thisSleepSEC>1) && (thisSleepSEC<10) || (thisSleepSEC>=10)
		? thisSleepSEC " Sekunden"
		: "") 
	;	.  "`t{"
	;	. subStr(ConvertToHHMMSSMS(atickcountB),1,8) " / " 		; UPCount
	;	. subStr(ConvertToHHMMSSMS(thisSleep-atickcountB),1,8) " / " 	; downCount
	;	. subStr(ConvertToHHMMSSMS(thisSleep),1,8)			; TimeStamp
	;	.  "}"
}
...
F1::						; activation der direktive für sonnenaufgang
atickcount := a_tickcount
DoIt := 1
;    ToggleGUI := 0
GUItimeout(.095)
...
F2::						; activation der direktive für sonnenuntergang
atickcount := a_tickcount
DoIt := 1
;    ToggleGUI := 0
GUItimeout(.095)
...
F3:: ; (122.25) (1440) 				; activation der direktive für Gefahrenmeldung
atickcount := a_tickcount
DoIt := 1
;    ToggleGUI := 0
GUItimeout(122.25) 
...
^F4:: 						; activation der direktive für ...
atickcount := a_tickcount
DoIt := 1
;    ToggleGUI := 0
GUItimeout(1440) 
...
runToolTip:
ToolTip %  "Debugmodus`n"
	 . ConvertToHHMMSSMS(Sonnenaufgang)  "`n"
	 . ConvertToHHMMSSMS(Sonnenuntergang)  "`n"
return

update:
GuiControl,Text,% hthisTEXTid, %thisGUIUpdate%
GuiControl,hide,% hthisTEXTtest
;GuiControl, Move,% hthisTEXTtest, W0 h0
;GuiControl, Disable,% hthisTEXTtest
sleep, 100
return 

;settimer, gefahrenmeldung_flash, 500
gefahrenmeldung_flash:
thisshow := (show:=!show)
if thisshow {
GuiControl, Enable,% hthisTEXTtest
sleep 1500
} else {
GuiControl, Disable,% hthisTEXTtest
}
...
;    ToggleGUI := 1
GuiControl,Text,% hthisTEXTid, %thisGUIUpdate%
Gui, Show, x110 y151,% hthisTEXTid a_space hthisTEXTid+0  
sleep,% thisSleep:=(min*60*1000)
Gui, Show, hide x110 y151,% hthisTEXTid a_space hthisTEXTid+0  
settimer, start, -1
;    ToggleGUI := 0
}
...
atickcount := a_tickcount
	...
	;	Gui, Show, x110 y151,% hthisTEXTid a_space hthisTEXTid+0  
		...
		fileAppend,% 		A_DD "." Month(A_MM) "." a_YYYY " Sonnenaufgang " A_Hour ":" A_Min ":" A_Sec " Uhr`n"
					, % a_scriptDir "\" A_ScriptName " timeStampall.txt"
		thisRISECounter++
	;	if !ToggleGUI
		GUItimeout(1440)  ; .1 ist 6 sekunden
		settimer, start, -1
		return " `tSUNRISE"
		} else {
	;		Gui, Show, x110 y151,% hthisTEXTid a_space hthisTEXTid+0  
			try run,% RISEfile,, hide, AusgabeVarPID
			timeStampRISEall .= 	A_DD "." Month(A_MM) "." a_YYYY " Sonnenaufgang " A_Hour ":" A_Min ":" A_Sec "`n"
			fileAppend,% 		A_DD "." Month(A_MM) "." a_YYYY " Sonnenaufgang TESTELSE " A_Hour ":" A_Min ":" A_Sec " Uhr`n"
						, % a_scriptDir "\" A_ScriptName " timeStampall.txt"
			thisRISECounter++
	;		if !ToggleGUI
			GUItimeout(1440)  ; .1 ist 6 sekunden
...
;	ToggleGUI := 0
	...
	return " `tSUNSET"
		...
		fileAppend,% 		A_DD "." Month(A_MM) "." a_YYYY " Sonnenuntergang " A_Hour ":" A_Min ":" A_Sec " Uhr`n"
					, % a_scriptDir "\" A_ScriptName " timeStampall.txt"
		thisSETCounter++
	;	if !ToggleGUI
		GUItimeout(1440)  ; (1440) .1 ist 6 sekunden
		settimer, start, -1
		return " `tSUNSET"
		} else {
			Gui, Show, x110 y151,% hthisTEXTid a_space hthisTEXTid+0  
			try run,% SETfile,, hide, AusgabeVarPID
			timeStampSETall .= 	A_DD "." Month(A_MM) "." a_YYYY " Sonnenuntergang " A_Hour ":" A_Min ":" A_Sec "`n"
			fileAppend,% 		A_DD "." Month(A_MM) "." a_YYYY " Sonnenuntergang TESTELSE " A_Hour ":" A_Min ":" A_Sec " Uhr`n"
						, % a_scriptDir "\" A_ScriptName " timeStampall.txt"
			thisSETCounter++
		;	; if !ToggleGUI
			GUItimeout(1440)  ; .1 ist 6 sekunden
