...
zoomfactor:=((A_ScreenWidth/1360)*(96/A_ScreenDPI))
...
;	Gui, Font, s%fs16% cYellow Bold, Verdana
	...
	;-
	Gui, Font, s%fs11% normal,
	x:=(wa*7)/xx,y:=(ha*5.2)/xx
	Gui, Add, text, x%x% y%y%, Stunden
	;-
	Gui, Font, s%fs10% normal,
	x:=(wa*12)/xx,y:=(ha*5)/xx,w:=(wa*1.6)/xx,h:=(ha*2.2)/xx
	Gui, Add, edit, x%x% y%y% w%w% h%h%  vSonnenEditMinuten hwndhSsonnenEditMinuten cWhite center, 59
	;-
	Gui, Font, s%fs11% normal,
	x:=(wa*14)/xx,y:=(ha*5.2)/xx
	Gui, Add, text, x%x% y%y% , Minuten
	;-
	Gui, Font, s%fs10% normal,
    ;-
	x:=(wa*18)/xx,y:=(ha*5)/xx,w:=(wa*10.5)/xx
	Gui, Add, DropDownList, x%x% y%y% w%w% r9 section vddl1 choose1, Trigger auswählen...
								| vor Sonnenaufgang
								| nach Sonnenaufgang
								| vor Sonnenuntergang
								| nach Sonnenuntergang
								| vor Winter>Sommerzeit Wechsel
								| nach Winter>Sommerzeit Wechsel
								| vor Sommer>Winterzeit Wechsel
								| nach Sommer>Winterzeit Wechsel
    ;-								
...
;	Gui, Add, listview, x10 ym+390 	w545 r7  vLV1 hwndhLV1 grid background%color% cBlack , Timestamp|ThisLabel|Name|ThisFunc|ThisHotkey|PriorHotkey|PriorKey|TimeSinceThisHotkey|TimeSincePriorHotkey|GuiControl|GuiEvent|EventInfo
...
;x:=(wa*10)/xx,y:=(ha*10)/xx,w:=(wa*30)/xx,h:=(ha*46)/xx
 x:=(wa*10)/xx,y:=(ha*10)/xx,w:=(wa*35)/xx,h:=(ha*80)/xx
